...
class MissionControlLayout:
    ...
    def create_layout(self):
        ...
        with dpg.window(tag="Primary Window", no_title_bar=True, no_move=True,
            ...
            # height=-110 rezerwuje dokładnie 110px na dolne panele sterowania[cite: 12]
            ...
            # --- 3. PRZYKLEJONY PANEL DOLNY (Poza obszarem main_content_area) ---
            with dpg.group(tag="fixed_bottom_panel"):
                ...
                # Pasek Mission Critical (50px)
                with dpg.child_window(height=50, border=False, no_scrollbar=True):
                    with dpg.group(horizontal=True):
                        with dpg.group(horizontal=True, width=400):
                            arm_btn = dpg.add_button(label="ARM & SYNC", width=130, tag="arm_btn")
                            dpg.bind_item_theme(arm_btn, theme.create_button_theme(theme.STATUS_BLUE))
                            disarm_btn = dpg.add_button(label="DISARM", width=100, tag="disarm_btn")
                            dpg.bind_item_theme(disarm_btn, theme.create_button_theme(theme.STATUS_AMBER))
                            reset_btn = dpg.add_button(label="RESET", width=100, tag="reset_btn")
                        ...
                        abort_btn = dpg.add_button(label="ABORT", width=90, tag="abort_btn")
